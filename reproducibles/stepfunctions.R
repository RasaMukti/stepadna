...
saveGif <- function(col_num, image_num, outName, diffusion_mat, timeP, Nx, Ny, lo1, lo2, la1, la2, x0, y0){
  ...
  colorBreaks = c(0, 1e-4, 3^(seq(-6, log10(1), length.out = col_num)))
  
  cols <- c("white", hcl.colors((length(colorBreaks) - 2L), "YlOrRd", rev=TRUE))
  ...
  saveGIF({
    ...
    for (tp in displayed_periods){
      ...
      cbar <- plot_discrete_cbar(round(colorBreaks, 4), colors=cols, spacing="constant", font_size = 4, legend_direction = "horizontal", expand_size = 0.1)
