...
timeP = round(opt$age/10) 
timeSplit = round(5000/290)

######################################## MLE ######################################## 

# Function for calculating likelihoods
LL_2step <- function(iter, Nx=Nx, Ny=Ny, s=s, sigmax=sigmax, sigmay=sigmay, D=D, x0=x0, y0=y0, xtran=xtran, ytran=ytran, timeP=timeP, timeSplit=timeSplit, oldnew=NULL){
...
#########################################################################################
start_time <- Sys.time()
ResOld <- param_optim_old(init_points = opt$init, seed = 5, num_cores = opt$cores)
CIres <- getCI(ResOld, "old")
...
ResNew <- param_optim_new(init_points = opt$init, seed = 5, num_cores = opt$cores)
CIresNew <- getCI(ResNew, "new")
